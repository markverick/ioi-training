...
//        printf("%lld %lld %lld %lld\n",a[i],b[i],c[i],d[i]);
        ...
        for(i=1;i<=tm;i++)
        {
            ...
            mic[i][2]=max(max(mic[i-1][1],mic[i-1][2]),max(mic[i-1][3],mic[i-1][4]))+b[i];
            mic[i][3]=max(mic[i-1][2],mic[i-1][3])+c[i];
            mic[i][4]=max(max(mic[i-1][1],mic[i-1][2]),max(mic[i-1][3],mic[i-1][4]))+d[i];
        }

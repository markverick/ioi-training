...
pair<long long,long long> jum[100135][5];
vector<pair<long long,long long> > adj[100135];
pair<long long,long long> dp(long long idx,long long can,long long p) ///return cost ways
{
    if(jum[idx][can].X>=0)
        return jum[idx][can];
    ...
    else
    {
        for(i=0;i<adj[idx].size();i++)
        {
            if(p==adj[idx][i].X)
                continue;
            tmp=dp(adj[idx][i].X,0,idx);
            tmp.X+=adj[idx][i].Y;
            prod=tmp.Y;sum=tmp.X;
            for(j=0;j<adj[idx].size();j++)
            {
                if(p==adj[idx][j].X||i==j)continue;
                a=dp(adj[idx][j].X,1,idx);
...
int main()
{
    ...
    pair<long long,long long> tmp=dp(1,1,0);

...
int n;
char maps[7][1135];
unordered_map<int,int> adj[6][2005][6],mat[6][2005][6];
int dx[6]={1,-1,0,0};
int dy[6]={0,0,1,-1};
...
//    freopen("out.txt","w",stdout);
    ...
    while(scanf("%d",&n)!=EOF)
    {
        ...
        for(i=0;i<=5;i++)
            for(j=0;j<=n*2+1;j++)
                for(k=0;k<=5;k++)
                    mat[i][j][k].clear(),adj[i][j][k].clear();
...
//            system("pause");
            ...
            {
                adj[vec[i].X][vec[i].Y][vec[i-1].X][vec[i-1].Y]--;
                adj[vec[i-1].X][vec[i-1].Y][vec[i].X][vec[i].Y]++;
            }
        }
        printf("%d\n",sum);
    }

    ...
    int main () {  
        ...
        while (scanf("%d", &N) == 1) {  
            ...
            for (int i = 0; i < N; i++) {  
                for (int j = 0; j < M; j++) {  
                    if (getIntersection(P[i], P[i+1]-P[i], Q[j], Q[j+1]-Q[j], T)) {  
                        if (onSegment(T, P[i], P[i+1]) && onSegment(T, Q[j], Q[j+1]))  
                            V.push_back(T);  
                    }  
                }  
            }  
            ...
            for (int i = 0; i < N; i++)  
                if (isPointInPolygon (P[i], Q, M) <= 0)  
                    V.push_back(P[i]);  
            for (int i = 0; i < M; i++)  
                if (isPointInPolygon (Q[i], P, N) <= 0)  
                    V.push_back(Q[i]);  
